...
{   
    ...
    if (TYPE(n) == small_stmt) {
        ...
        */
        switch (TYPE(n)) {
            case expr_stmt:
                ret = ast_for_expr_stmt(c, n);
                goto handler_dealer;
            case del_stmt:
                ret = ast_for_del_stmt(c, n);
                goto handler_dealer;
            case pass_stmt:
                ret = Pass(LINENO(n), n->n_col_offset, c->c_arena);
                goto handler_dealer;
            case flow_stmt:
                ret = ast_for_flow_stmt(c, n);
                goto handler_dealer;
            case import_stmt:
                ret = ast_for_import_stmt(c, n);
                goto handler_dealer;
            case global_stmt:
                ret = ast_for_global_stmt(c, n);
                goto handler_dealer;
            case nonlocal_stmt:
                ret = ast_for_nonlocal_stmt(c, n);
                goto handler_dealer;
            case assert_stmt:
                ret = ast_for_assert_stmt(c, n);
                goto handler_dealer;
            default:
                PyErr_Format(PyExc_SystemError,
        ...
        */
        ...
        switch (TYPE(ch)) {
            case if_stmt:
                ret = ast_for_if_stmt(c, ch);
                goto handler_dealer;
            case while_stmt:
                ret = ast_for_while_stmt(c, ch);
                goto handler_dealer;
            case for_stmt:
                ret = ast_for_for_stmt(c, ch, 0);
                goto handler_dealer;
            case try_stmt:
                ret = ast_for_try_stmt(c, ch);
                goto handler_dealer;
            case with_stmt:
                ret = ast_for_with_stmt(c, ch, 0);
                goto handler_dealer;
            case funcdef:
                ret = ast_for_funcdef(c, ch, NULL);
                goto handler_dealer;
            case classdef:
                ret = ast_for_classdef(c, ch, NULL);
                goto handler_dealer;
            case decorated:
                ret = ast_for_decorated(c, ch);
                goto handler_dealer;
            case async_stmt:
                ret = ast_for_async_stmt(c, ch);
                goto handler_dealer;
            default:
                PyErr_Format(PyExc_SystemError,
                             "unhandled small_stmt: TYPE=%d NCH=%d\n",
