...
#ifdef Py_DEBUG
static
int childrenprintf(const node* n, int num){
    num++;
    int N = NCH(n);
    printf("[search:%d]=> %d \t",num,TYPE(n));
    if (N==0){
        return 0;
    }
    else{
        int i=0;
        for(;i<N; ++i) childrenprintf(CHILD(n,i),num);
    }
    return 0;
}
#endif
...
{   
    ...
    body   = ast_for_suite    (c, CHILD(where_handler, 6) );
    if(!body  ) return NULL;
...
ast_for_stmt(struct compiling *c, const node *n)
{
    ...
    else if (TYPE(n) == expr_stmt_wrapper ){
        #ifdef Py_DEBUG
            printf("in_expr_stmt:debugging");
            childrenprintf(CHILD(n,0), 0);
            printf("\n");
        #endif
        return ast_for_expr_stmt(c, CHILD(n,0));
    }
    else {
