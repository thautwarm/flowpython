...
static expr_ty 
try_pattern_matching(struct compiling *c, const node *n, expr_ty target, expr_ty to_match, expr_ty to_match_augs, expr_ty func)
{
    ...
    // asdl_seq_SET(try_handler, 0, do_if);
    asdl_seq_SET(try_handler, 0, do_assign);
    
    //===========

    // // do_except
    // do_except  = Assign(tmp_var_seq, _py_false, LINENO(n)+5, n->n_col_offset+2, c->c_arena);
    // try_handler = _Py_asdl_seq_new(1, c->c_arena);
    // asdl_seq_SET(try_handler, 0, do_except);

    // // do_if
    // if (to_match_augs == NULL){
    //     do_if = Assign(tmp_var_seq, _py_true, LINENO(n)+3, n->n_col_offset+2, c->c_arena);
    // }
    // else{
    //     cmpop_ty _op;
    //     _op = Eq;
    ...
    //     if (func != NULL){
    //         test_call  = Call(func, to_match_seq, NULL, LINENO(n)+3, n->n_col_offset+2, c->c_arena);
    //         if_test_res = Compare(test_call , ops, to_match_augs_seq, LINENO(n)+3, n->n_col_offset+2, c->c_arena);
    //     }
    //     else{
    //         if_test_res = Compare(to_match, ops, to_match_augs_seq, LINENO(n)+3, n->n_col_offset+2, c->c_arena);
    //     }
    //     do_if_exp = IfExp(if_test_res, _py_true, _py_false, LINENO(n)+3, n->n_col_offset+2, c->c_arena);
    //     do_if = Assign(tmp_var_seq, do_if_exp, LINENO(n)+3, n->n_col_offset+2, c->c_arena);
    // }
    ...
    // // do_try
    // do_try = Try(try_body, try_handler, NULL, NULL, LINENO(n)+1, n->n_col_offset+1, c->c_arena);
    // where_suite = _Py_asdl_seq_new(1, c->c_arena);
    // asdl_seq_SET(where_suite, 0, do_try);

    // set context
    // tmp_var->v.Name.ctx = Store;
    if (!set_context(c, to_match, Store, CHILD(n, 1)))
        return NULL;
    
    // return Where(_py_true, try_body,  LINENO(n), n->n_col_offset, c->c_arena);
    return Where(_py_true, try_handler, LINENO(n), n->n_col_offset, c->c_arena);
}
